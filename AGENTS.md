...
- Before create PR: new branch from `main`, read docs (README.md and all md files in docs folder), validate them against the code, update whichever side (docs or code) is out of sync so they stay consistent, tests for new code (including automation tests in `automation/tests/` when adding or modifying API endpoints and UI), acceptance criteria in `docs/TEST_STRATEGY.md` passes, match code style (e.g. named exports, no semicolons, vitest).

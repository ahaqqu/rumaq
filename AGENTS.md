...
- `scripts/test-unit.sh`, `scripts/test-api.sh`, `scripts/test-e2e.sh`, `scripts/test-live-e2e.sh` are the entrypoints for tests.
...
- Before create PR: new branch from `main`, read docs (README.md and all md files in docs folder), validate them against the code, update whichever side (docs or code) is out of sync so they stay consistent, tests for new code (including BDD integration tests in `automation/tests/local/api/` when adding or modifying API endpoints), `./scripts/test-unit.sh` passes, match code style (e.g. named exports, no semicolons, vitest).

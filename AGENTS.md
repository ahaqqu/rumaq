...
- Before create PR: new branch from `main`, read docs (README.md and all md files in docs folder), validate them against the code, update whichever side (docs or code) is out of sync so they stay consistent, meet all acceptance criterias in `docs/TEST_STRATEGY.md` passes, run `./scripts/style.sh` to auto-format and lint-fix frontend, backend, shell, and docs (`./scripts/style.sh --check` to verify without writing), and run `./scripts/test-unit.sh`.
